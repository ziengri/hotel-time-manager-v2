...
func set_status(new_status):
	if(new_status == RoomStatus.DIRTY):
		EventManager.add_event(self,EventManager.event_infos['other_events']['room_cleaning'])
	status = new_status
...
func start_event(event_info : EventInfo,visitor:CharacterBody2D) ->Dictionary:
	# var event_in_list :Dictionary = 
	return EventManager.add_event(self,event_info,visitor)
